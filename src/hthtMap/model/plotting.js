const ctxColorText = 'white'
const WSclor = 'black'// white
const Nclor = 'black'// white
const CLclor = 'black'// white
...
export default class Plotting {
  constructor () {
    ...
    this.ctxObj = this.canvas.getContext('2d')
  }
   ...
   * @param coe 数据缩放系数
   */
  drawGroundAll (staInfo, coe = 1) {
    const ctx2d = this.ctxObj
    ...
    const dataInfo = objDeepCopy(staInfo.dataInfo)

    // 无效数据不绘制
    if (dataInfo.InvalidType) {
      for (const i in dataInfo.InvalidType) {
        dataInfo[dataInfo.InvalidType[i]] = dataInfo.INVALID_VALUE
      }
    }

    const { width } = ctx2d.canvas
    const { height } = ctx2d.canvas// canvas宽高
    const scale = 0.3// 绘图类缩放系数
    const x = 0
    const y = -7// 基础XY y为字体的一半
    const xStep = width / 6
    const yStep = height / 10

    const colorText = ctxColorText
    ...
    if (dataInfo.TD !== dataInfo.INVALID_VALUE) {
      ...
      const TD = Math.round(dataInfo.TD / coe)
    ...
    if (dataInfo.OP !== dataInfo.INVALID_VALUE) {
      ...
      this.drawText(ctx2d, OP, colorText, '10px sans-serif', myPoint(x + 5 * xStep, y + 3 * yStep),
        width)
    }
    // 3小时变压
    ...
    if (dataInfo.WD !== dataInfo.INVALID_VALUE && dataInfo.WS !== dataInfo.INVALID_VALUE) {
      ...
      const WS = Math.floor(dataInfo.WS / coe)
      this.drawWindShape(1, ctx2d, WD, WS, width / 3, myPoint(x + 3 * xStep, y + 5.5 * yStep),
        'black')
    }
   ...
   * @param coe 数据缩放系数
   */
  drawGroundJD (staInfo, ctx2d, ctxColorText = 'white', coe = 1) {
    if (!staInfo || !staInfo.dataInfo || !ctx2d) {
      return
    }
   ...
   * @param ctx2d canvas的2d画布
   */
  drawHigh (staInfo, ctx2d, ctxColorText = 'white', coe = 1) {
    if (!staInfo || !staInfo.dataInfo || !ctx2d) {
      return
    }
    ...
    if (dataInfo.PS !== dataInfo.INVALID_VALUE) {
      const PS = Math.round(dataInfo.PS / coe)
   ...
   * @param ctx2d canvas的2d画布
   */
  drawGroundFB (staInfo, ctx2d, ctxColorText = 'white', coe = 1) {
    if (!staInfo || !staInfo.dataInfo || !ctx2d) {
      return
    }
    ...
    const xp = 2
    const yp = 2
    const ly = y + (ht - height) / 2
    const lyp = yp + (ht - height)
    // const color = 'black'
    const colorText = ctxColorText
    ...
    this.drawCloudAmount(ctx2d, 9,
      myPoint(x + width + xp + ht / 2 - 9, y + 1.5 * (ht + yp) + ht / 2 - 9), ht / 2)
    //  浮标移动方向及速度符号
    if (dataInfo.SD !== dataInfo.INVALID_VALUE && dataInfo.SS !== dataInfo.INVALID_VALUE) {
      ...
      const SS = Math.floor(dataInfo.SS / coe)
      this.drawWindShape(1, ctx2d, SD, SS, ht / 2 + width,
        myPoint(x + width + xp + ht / 2, y + 1.5 * (ht + yp) + ht / 2), 'black')
    }
    ...
    if (dataInfo.SST !== dataInfo.INVALID_VALUE) {
      ...
      const SST = Math.round(dataInfo.SST / coe)
      this.drawText(ctx2d, SST, colorText, font, myPoint(x0, ly), width, true)
    }
    //  平均波高
    if (dataInfo.V22381 !== dataInfo.INVALID_VALUE) {
      const V22381 = dataInfo.V22381 / coe
      this.drawText(ctx2d, V22381, colorText, font, myPoint(x0, ly + 3 * (height + lyp)), width,
        true)
    }
   ...
   */
  drawText (ctx2d, info, color, font, xy, width) {
   ...
   * @param size 符号半径
   */
  drawCloudAmount (ctx2d, amount, xy, size) {
    ctx2d.strokeStyle = Nclor
    ctx2d.fillStyle = Nclor

    // const ptLeftTop = myPoint(xy.X, xy.Y)
    // 圆心、半径
    xy.X += size
    xy.Y += size
    const d = size - size / 8.0
    ...
    // 上、下、左、右四点坐标
    const ptf1 = myPoint(xy.X, xy.Y - d)
    const ptf2 = myPoint(xy.X, xy.Y + d)
    const ptf3 = myPoint(xy.X - d, xy.Y)
    const ptf4 = myPoint(xy.X + d, xy.Y)
    ...
    switch (amount) {
      ...
      default: // 不明
        ...
        const cosd = d * Math.cos(45 * Math.PI / 180)
        // eslint-disable-next-line no-case-declarations
        const sind = d * Math.sin(45 * Math.PI / 180)
        ptf1.X = xy.X - cosd
        ptf1.Y = xy.Y - sind
        ptf2.X = xy.X + cosd
        ptf2.Y = xy.Y + sind
   ...
   * @param isWind 静风标识
   */
  drawCloudAmountJD (ctx2d, amount, xy, size, isWind) {
    ctx2d.strokeStyle = 'white'
  ...
  drawWindBarbJD (ctx2d, dd, ff, size, xy, color) {
    ...
    // 风杆
    const ptf1 = myPoint(xy.X, xy.Y)
    const ptf2 = myPoint(xy.X + size * sind, xy.Y - size * cosd)
  ...
  drawWindBarb (ctx2d, dd, ff, size, xy, color) {
    ...
    // 风杆
    const ptf1 = myPoint(xy.X, xy.Y)
    const ptf2 = myPoint(xy.X + 8 * dw0 * sind, xy.Y - 8 * dw0 * cosd)
    ...
    const dpcha = dw0 * 2.0 * Math.tan(Math.PI / 3.0)
    const ptf3 = myPoint(0, 0)
  ...
  drawWindArrow (ctx2d, dd, ff, size, xy, color) {
    ...
    // 风杆
    const ptf1 = myPoint(xy.X, xy.Y)
    const ptf2 = myPoint(xy.X + dw * sind, xy.Y - dw * cosd)
    ...
    const ptf4 = myPoint(ptf1.X + (dw - dw0) * sind, ptf1.Y - (dw - dw0) * cosd)
    ...
    const ptf3 = myPoint(ptf4.X - dpcha * cosd, ptf4.Y - dpcha * sind)
   ...
   */
  drawClShape (ctx2d, code, xy, size) {
    if (size < 0.2) {
      size = 0.2
    }
    ...
    const wdht = size * 60
    const ptf = myPoint(xy.X + wdht / 2.0, xy.Y + wdht / 2.0)
    const r = wdht / 2.0
    const d = wdht / 4.0
    const sin30d = d * Math.sin(Math.PI / 6.0)
    const cos30d = d * Math.cos(Math.PI / 6.0)
    const sin45d = d * Math.sin(Math.PI / 4.0)
    const cos45d = d * Math.cos(Math.PI / 4.0)
    const sin60d = d * Math.sin(Math.PI / 3.0)
    const cos60d = d * Math.cos(Math.PI / 3.0)

    const ptf0 = myPoint(0, 0)
    ctx2d.beginPath()
    switch (code) {
      case 1:
      ...
      case 9:
        ...
        ctx2d.lineTo(xy.X, ptf.Y + wdht / 6.0 + d)
        ctx2d.arc(ptf.X, ptf.Y + wdht / 6.0 + d, r, Math.PI, 0)
        ctx2d.moveTo(ptf.X - r * Math.cos(Math.PI / 3),
          ptf.Y + wdht / 6.0 + d - r * Math.sin(Math.PI / 3))
        ctx2d.lineTo(xy.X + wdht / 6.0, xy.Y + wdht / 6.0)
        ctx2d.lineTo(xy.X + wdht - wdht / 6.0, xy.Y + wdht / 6.0)
        ctx2d.lineTo(ptf.X + r * Math.cos(Math.PI / 3),
          ptf.Y + wdht / 6.0 + d - r * Math.sin(Math.PI / 3))
        ctx2d.stroke()
        break
    }
   ...
   * @param size 符号大小缩放率 60为基数 直径为scale*60
   */
  drawCmShape (ctx2d, code, xy, size) {
    if (size < 0.2) {
      size = 0.2
    }
   ...
   * @param size 符号大小缩放率 60为基数 直径为scale*60
   */
  drawChShape (ctx2d, code, xy, size) {
    if (size < 0.2) {
      size = 0.2
    }
   ...
   * @param size 符号大小缩放率 60为基数 直径为scale*60
   */
  drawWeathShape (ctx2d, whType, code, xy, size) {
  ...
  drawWWShape (ctx2d, code, xy, size) {
    if (size < 0.2) {
      size = 0.2
    }
    ...
    switch (code) {
      ...
      case 4:
        ...
        dx = (wdht - 2 * dx1) / 10.0
        // eslint-disable-next-line no-case-declarations
        const ptfxy = myPoint(xy.X + dx1, xy.Y + dx1)
      ...
      case 7:
        ...
        break
      case 8:
        ctx2d.arc(ptf.X, ptf.Y - wdht / 6.0, wdht / 6.0, -Math.PI / 2 + Math.PI / 12, Math.PI / 6,
          true)
        ctx2d.arc(ptf.X, ptf.Y, wdht / 6.0, -Math.PI / 6, Math.PI / 6, true)
        ctx2d.arc(ptf.X, ptf.Y + wdht / 6.0, wdht / 6.0, -Math.PI / 6, Math.PI / 2 - Math.PI / 12,
          true)
        ctx2d.stroke()
        break
      case 9:
      ...
      case 20:
        ...
        ctx2d.closePath()
        ctx2d.beginPath()
        ctx2d.arc(ptf.X - wdht / 8.0 + wdht / 24.0, ptf.Y - wdht / 12.0 + wdht / 24.0, wdht / 24.0,
          0, Math.PI * 2, false)
        ctx2d.fill()
        break
      case 21:
      ...
      case 23:
        ...
        ctx2d.closePath()
        ctx2d.beginPath()
        ctx2d.arc(ptf.X - wdht / 12.0, ptf.Y - wdht / 4.0 + wdht / 12.0, wdht / 12.0, 0,
          Math.PI * 2, false)
        ctx2d.fill()
        break
      case 24:
        ...
        ctx2d.arc(xy.X + wdht - wdht / 4.0 - wdht / 8.0, ptf.Y, wdht / 8.0, -Math.PI / 6, Math.PI,
          false)
        ctx2d.arc(xy.X + wdht - wdht / 2.0 - wdht / 8.0, ptf.Y, wdht / 8.0, 0,
          Math.PI - Math.PI / 6, true)
        ctx2d.stroke()
        break
      case 25:
        ctx2d.moveTo(ptf.X - wdht / 6.0, ptf.Y)
      ...
      case 51:
        ...
        ctx2d.closePath()
        ctx2d.beginPath()
        ctx2d.arc(ptf.X - wdht / 8.0 + wdht / 24.0, ptf.Y - wdht / 24.0, wdht / 24.0, 0,
          Math.PI * 2, false)
      ...
      case 53:
        ...
        ctx2d.beginPath()
        ctx2d.arc(ptf.X, ptf.Y - wdht / 24.0 - wdht / 8.0, wdht / 24.0, 0, Math.PI * 2, false)
        ctx2d.arc(ptf.X - wdht / 8.0 + wdht / 24.0, ptf.Y - wdht / 24.0 + wdht / 8.0, wdht / 24.0,
          0, Math.PI * 2, false)
        ctx2d.fill()
        ctx2d.closePath()
        ctx2d.beginPath()
        ctx2d.arc(ptf.X + wdht / 12.0, ptf.Y - wdht / 24.0 + wdht / 8.0, wdht / 24.0, 0,
          Math.PI * 2, false)
        ctx2d.fill()
        break
      case 54:
      ...
      case 56:
        ...
        ctx2d.closePath()
        ctx2d.beginPath()
        ctx2d.arc(ptf.X - 7.0 * d + wdht / 24.0, ptf.Y - 2.0 * d + wdht / 24.0, wdht / 24.0, 0,
          Math.PI * 2, false)
        ctx2d.fill()
        break
      case 57:
        ...
        ctx2d.arc(ptf.X + wdht / 8.0 + wdht / 8.0, ptf.Y - wdht / 24.0, wdht / 24.0, 0, Math.PI * 2,
          false)
        ctx2d.fill()
        break
      case 58:
  ...
  drawWWShapeJD (ctx2d, code, xy, size) {
    if (code < 4 || code > 100) {
      return
    }
    ...
    if ((code >= 10 && code <= 12) || (code >= 40 && code <= 47) || code === 28 || code === 48 ||
      code === 49) {
    ...
    } else if ((code >= 87 && code <= 90) || (code >= 91 && code <= 95) || code === 56 || code ===
      57 || code === 66 || code === 67 ||
      code === 27 || code === 13 || code === 18 || code === 19 || code === 17 || code === 29 ||
      code ===
      97 || code === 96 || code === 99 || code === 98) {
      ctx2d.strokeStyle = 'red'
      ctx2d.fillStyle = 'red'
    } else {
    ...
    switch (code) {
      ...
      case 99:
        ...
        break
      case 98:
        ctx2d.arc(ptf.X + wdht / 12.0, ptf.Y - wdht * 5 / 12.0 - wdht / 8.0, wdht / 8.0, 0, Math.PI,
          true)
        ctx2d.arc(ptf.X + wdht / 12.0, ptf.Y - wdht * 5 / 12.0 + wdht / 8.0, wdht / 8.0,
